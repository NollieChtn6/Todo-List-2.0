...
export const createNewTask = async (req: Request, res: Response) => {
	const { title, description, tags } = tasksSchemas.updateTaskSchema.parse(
		req.body,
	);
	...
	res
		.status(201)
		.json({ message: "Task has successfully been created:", newTask });
};
...
export const updateTask = async (req: Request, res: Response) => {
	...
	res
		.status(200)
		.json({ message: "Task has successfully been updated", updatedTask });
};

export const updateTaskStatus = async (req: Request, res: Response) => {
	const taskId = Number(req.params.id);
	const { isComplete } = tasksSchemas.updateTaskStatusSchema.parse(req.body);

	const updatedTaskStatus = await tasksServices.updateTaskStatus(taskId, {
		isComplete,
	});

	res.status(200).json({
		message: "Task status has successfully been updated",
		updatedTaskStatus,
	});
};
